...
REST 리소스와 HTTP Method를 기본으로 하되 경로의 의미 전달을 우선한다. 복합 명사는 kebab-case를 허용하고 `prefill`, `lookup`처럼 화면 목적이 분명한 경로도 유지한다. 지원자 흐름도 같은 `/api/v1` 계약에 포함한다.
