...
공고 생성 시 완성된 기본 정보를 함께 저장한다. DRAFT와 `UPCOMING`에서는 모든 섹션, `OPEN`에서는
공고명과 모집 종료 시각의 연장만 허용하며 `RECRUIT_CLOSED`에서는 모든 수정을 막는다. 동시 수정 제어는 실제 충돌
가능성과 사용자 경험을 검토한 뒤 도입한다. 배역·일정·지원 폼과 게시 API는 후속 단계에서 추가한다.
