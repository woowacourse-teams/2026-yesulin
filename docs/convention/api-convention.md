...
- 지원 안내는 최대 2,000자다. 지원 폼은 선택한 기본 정보(필수), 선택한 추가 정보(nullable), 사진 설명 최대 255자의 `{description, count}` 배열(기본 1장·합계 최대 10), 영상 설명 최대 255자의 `{description}` 배열(최대 10), 질문 문구 최대 255자·답변 최대 2,000자·필수 여부를 가진 텍스트 커스텀 질문으로 구성한다. 새 공고의 영상 요구 배열은 비어 있다.
...
- 기획사/제작사용 상세는 권한을 확인하고 배우의 민감 정보와 해당 차수 심사 기록을 반환한다.
...
프런트·MSW는 아직 왼쪽 `/api/**` 계약을 사용한다. 현재 목 `PATCH /api/me/profile`은 정보 답변과 사진·영상 보관함 배열을 함께 받을 수 있지만 실제 바이너리 업로드 계약은 아니다. 배우 소셜 로그인은 실제 OAuth API를 호출하지 않고 React 상태에 불투명한 프론트 전용 자격값을 저장해 라우트 이동 동안 재사용한다. 새로고침 복원과 실제 세션·토큰 계약은 구현하지 않았다. 연동 기능을 구현할 때 이 문서, flowchart, 클라이언트와 MSW를 같은 작업에서 갱신한다.
