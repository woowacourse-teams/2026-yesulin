...
- 도메인별 파일 연결은 각 도메인이 `fileId`를 참조하고 자신의 사건을 발행한다. presentation event adapter가 파일 application을 호출해 소유자와 `READY` 상태를 확인하고 범용 `FileReference`를 추가·교체한다. 파일 서비스에 `attachPerformancePoster` 같은 도메인 전용 메서드나 `ATTACHED` 상태를 추가하지 않는다.
...
- `FileReference`: `fileId`, `referenceType`, `referenceId`, `referenceSlot`로 실제 사용 관계를 기록한다. 대상·위치 조합은 하나의 파일만 가리키며 파일 하나는 여러 관계에서 재사용할 수 있다.
...
공연 생성은 공연 ID와 포스터 파일 ID를 가진 `PerformanceCreatedEvent`, 포스터 교체는 공연 ID와 이전·신규 파일 ID를 가진 `PerformancePosterChangedEvent`를 발행한다. presentation의 `PerformanceFileEventHandler`는 `BEFORE_COMMIT`에 참조를 추가하거나 교체한다. 신규 파일의 소유자나 `READY` 상태가 유효하지 않으면 공연 트랜잭션도 롤백한다. 파일 application/domain은 공연별 규칙을 모르고 adapter가 전달한 범용 참조 키만 처리한다.
