...
public record SaveScreeningReviewsCommand(
    ...
    public SaveScreeningReviewsCommand {
        ...
        if (applicationIds.stream().anyMatch(Objects::isNull)) {
            throw new BusinessException(INVALID_REVIEW, "지원서 ID는 필수입니다.");
        }
        if (new HashSet<>(applicationIds).size() != applicationIds.size()) {
