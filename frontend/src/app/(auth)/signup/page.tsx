...
export default async function SignupPage({ searchParams }: { readonly searchParams: Promise<{ role?: string; claim?: string }> }) {
  const { role, claim } = await searchParams;
  return (
    <AuthShell
    ...
    >
      <SignupForm initialRole={role === "producer" ? "producer" : "applicant"} profileClaimToken={claim} />
    </AuthShell>
  );
}
