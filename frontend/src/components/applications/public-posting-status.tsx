...
export function PublicPostingUnavailable() {
  return (
    <main className="min-h-screen bg-surface px-5 py-16 sm:px-8">
      <section className="mx-auto max-w-[680px] rounded-modal border border-border bg-card px-6 py-14 text-center shadow-[var(--shadow-1)]">
        ...
        <Link href="/" className="mt-7 inline-flex min-h-11 items-center justify-center rounded-control bg-brand px-5 font-semibold text-white transition-colors hover:bg-brand-strong">
          예술in 홈으로 가기
        </Link>
      </section>
    </main>
...
export function PostingStatusBadge({ status }: { status: "OPEN" | "UPCOMING" | "CLOSED" }) {
  ...
  return <span className={`inline-flex rounded-full border px-3 py-1 text-sm font-semibold ${tone}`}>{label}</span>;
}
