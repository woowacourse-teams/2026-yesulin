...
import { lookupApplication } from "@/features/applicants/api";
...
import type { LookupApplicationResponse } from "@/features/applicants/types";
import { FieldInput } from "@/components/auditions/ui-controls";

export function PublicApplicationLookup() {
  const [code, setCode] = useState("");
  const [phone, setPhone] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [result, setResult] = useState<LookupApplicationResponse | null>(null);
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    try {
      setResult(await lookupApplication({ code, phone }));
    } catch (cause) {
