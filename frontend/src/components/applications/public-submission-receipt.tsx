  ...
  return <main className="min-h-screen bg-surface px-5 py-10 text-foreground md:px-8 md:py-14">
    <div className="mx-auto max-w-[880px]">
      <section className="rounded-card border border-border bg-card px-5 py-9 text-center md:px-10 md:py-11">
        ...
        <p className="mt-3 leading-7 text-muted-strong">{meta.applicationType === "OTR" ? "OTR 공고 지원이 정상적으로 접수됐어요." : "제출한 내용은 계정의 내 지원서에서 읽기 전용으로 확인할 수 있어요."}</p>
        ...
        <section aria-label="지원 완료 정보" className="mt-8 rounded-card border border-border bg-surface p-5 text-left md:p-6">
          <dl className="grid gap-x-4 gap-y-3 text-sm md:grid-cols-[96px_1fr]">
            ...
            <dt className="text-muted">선택 배역</dt><dd className="font-medium">{meta.applicationType === "OTR" ? state.values.SELECTED_ROLE : meta.roleName}</dd>
            <dt className="text-muted">제출 시각</dt><dd className="num font-medium">{formatApplicantDate(receipt.submittedAt, true)}</dd>
            {meta.applicationType === "OTR" ? <><dt className="text-muted">접수 번호</dt><dd className="num font-medium">#{receipt.submissionId}</dd></> : null}
          </dl>
          {meta.applicationType === "STANDARD" ? <PrimaryLink href={applicantRoutes.submission(submissionId(receipt.submissionId))} className="mt-5 w-full">내 지원서에서 확인</PrimaryLink> : null}
        </section>
