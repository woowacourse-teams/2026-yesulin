...
import { getScreeningBoard } from "@/features/screening/api";
import { screeningRoutes } from "@/features/screening/routes";
import type { RoleId, RoundNumber, ScreeningBoardResponse } from "@/features/screening/types";
...
export function ScreeningBoard({ roleId }: { roleId: RoleId }) {
  const [round, setRound] = useState<RoundNumber | null>(null);
  /** 심사·마감 응답으로 갱신된 보드. 조회 결과보다 우선하고, 차수가 바뀌면 무효가 된다. */
  const [applied, setApplied] = useState<{ round: RoundNumber; board: ScreeningBoardResponse } | null>(
    null,
  );
  ...
  const applyBoard = useCallback((next: ScreeningBoardResponse) => {
