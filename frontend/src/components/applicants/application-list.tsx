...
import type { ApplicantApplicationDraftSummary, ApplicantApplicationSummary, ApplicantRoleProgress } from "@/features/applicants/types";
...
export function ApplicantApplicationList() {
  ...
  useEffect(() => {
    ...
    let active = true;
    const submittedPostingIds = new Set(query.data.applications.map((application) => application.postingId));
  ...
  const empty = !draftsLoading && drafts.length === 0 && applications.length === 0;

  return <Container>
    <header><p className="text-sm font-semibold text-brand">내 지원서</p><h1 className="mt-2 text-[clamp(28px,4vw,38px)] font-bold tracking-[-0.035em]">지원 준비부터 전형 진행까지 확인하세요.</h1><p className="mt-3 max-w-2xl leading-7 text-muted-strong">이 기기에서 작성 중인 지원서와 제출 완료된 지원서의 배역별 전형 상황을 한곳에서 확인할 수 있어요.</p><div className="mt-5 flex flex-wrap gap-2"><SummaryChip label="작성 중" count={drafts.length} loading={draftsLoading} /><SummaryChip label="제출 완료" count={applications.length} /></div></header>
    {draftsLoading || drafts.length ? <section aria-labelledby="draft-applications-title" className="mt-10"><SectionHeader id="draft-applications-title" eyebrow="이 기기에 저장됨" title="작성 중" detail="제출 전 내용은 기획사/제작사에 공개되지 않아요." />{draftsLoading ? <div className="mt-4 h-40 animate-pulse rounded-card bg-border-soft" /> : <ul className="mt-4 grid gap-4">{drafts.map((draft) => <DraftCard key={draft.postingId} draft={draft} />)}</ul>}</section> : null}
    {applications.length ? <section aria-labelledby="submitted-applications-title" className="mt-10"><SectionHeader id="submitted-applications-title" eyebrow="읽기 전용 스냅샷" title="제출 완료" detail="전형 결과는 각 배역의 차수가 마감된 뒤 표시됩니다." /><ul className="mt-4 grid gap-4">{applications.map((application) => <SubmittedCard key={application.id} application={application} />)}</ul></section> : null}
    {empty ? <EmptyApplications /> : null}
  </Container>;
}
...
function SubmittedCard({ application }: { readonly application: ApplicantApplicationSummary }) {
  return <li><Link href={applicantRoutes.application(application.id)} className="group grid gap-5 rounded-card border border-border bg-card p-5 transition-[border-color,box-shadow] hover:border-brand-line hover:shadow-[var(--shadow-1)] sm:grid-cols-[88px_minmax(0,1fr)_auto] sm:items-center md:p-6"><Image src={application.posterUrl} alt={`${application.performanceTitle} 포스터`} width={88} height={116} unoptimized className="h-[116px] w-[88px] rounded-control object-cover" /><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><StatusBadge label="제출 완료" tone="border-pass/25 bg-pass-bg text-pass" /><span className="text-xs text-muted">{application.companyName}</span></div><h2 className="mt-3 truncate text-lg font-bold group-hover:text-brand">{application.performanceTitle}</h2><p className="mt-1 truncate text-sm text-muted-strong">{application.postingTitle}</p><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-surface px-2.5 py-1 font-semibold text-muted-strong">지원서 1개</span><span className="rounded-full bg-surface px-2.5 py-1 font-semibold text-muted-strong">지원 배역 {application.roleProgress.length}개</span></div><RoleProgressList roles={application.roleProgress} /><p className="num mt-3 text-xs text-muted">{formatApplicantDate(application.submittedAt, true)} 제출</p></div><span className="inline-flex min-h-11 items-center justify-center self-end rounded-control border border-border px-4 text-sm font-semibold text-muted-strong group-hover:border-brand-line group-hover:bg-brand-soft group-hover:text-brand sm:self-center">상세 보기</span></Link></li>;
}
