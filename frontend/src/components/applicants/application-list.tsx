...
function EmptyApplications({ filtered }: { readonly filtered: boolean }) {
  return <section className="mt-5 rounded-card border border-dashed border-border bg-card px-6 py-14 text-center"><span aria-hidden="true" className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-brand-soft text-xl text-brand">▤</span><h2 className="mt-5 text-lg font-bold">{filtered ? "이 상태의 지원서가 없어요" : "아직 지원한 공고가 없어요"}</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted">{filtered ? "다른 필터를 선택해 전체 지원서를 확인해 주세요." : "공연사가 공유한 예술in 지원 링크에서 제출하면 이곳에 자동으로 모입니다."}</p><Link href={applicantRoutes.lookup} className="mt-6 inline-flex min-h-11 items-center rounded-control border border-border px-4 text-sm font-semibold hover:border-brand-line hover:bg-brand-soft hover:text-brand">비로그인 지원서 찾기</Link></section>;
}
