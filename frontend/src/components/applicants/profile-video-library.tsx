...
export const MAX_LIBRARY_VIDEOS = 3;
...
export function ProfileVideoLibrary({ profile, onSaved }: { readonly profile: ApplicantProfileResponse; readonly onSaved: (profile: ApplicantProfileResponse) => void }) {
  ...
  const add = async () => {
    ...
    if (videos.length >= MAX_LIBRARY_VIDEOS) { setError(`영상은 최대 ${MAX_LIBRARY_VIDEOS}개까지 보관할 수 있어요.`); return; }
    if (await persist(
  ...
  return <div className="mt-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="font-bold">영상 보관함</h3><p className="mt-1 text-sm leading-6 text-muted">YouTube 공개 또는 일부공개 링크를 최대 {MAX_LIBRARY_VIDEOS}개까지 보관할 수 있어요.</p></div><span className="num rounded-full bg-brand-soft px-3 py-1 text-sm font-semibold text-brand">{videos.length} / {MAX_LIBRARY_VIDEOS}</span></div>
