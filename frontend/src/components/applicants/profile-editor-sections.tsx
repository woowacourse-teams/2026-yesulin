...
import { formatKoreanPhone, isValidEmail, isValidKoreanPhone } from "@/features/applicants/profile-input";
import type { ApplicationFieldInput } from "@/features/auditions/creation-types";
import { CalendarDateRangeField } from "@/components/auditions/calendar-date-range-field";
import { AddButton, DestructiveButton, FieldInput, FieldSelect, FieldTextarea, TextButton } from "@/components/ui/controls";
...
function StandardField({ field, value, required, onChange }: { readonly field: ApplicationFieldInput; readonly value?: ApplicantAnswerValue; readonly required: boolean; readonly onChange: (value: ApplicantAnswerValue) => void }) {
  ...
  const fieldError = contactError || emailError;
  ...
  return <label htmlFor={id} className={width}><span className="mb-2 block text-sm font-semibold">{label}</span>{field.inputType === "TEXTAREA" ? <><FieldTextarea id={id} required={required} rows={6} maxLength={field.config.maxLength} value={text} placeholder={field.config.placeholder} onChange={(event) => onChange(event.target.value)} />{field.config.maxLength ? <span className="num mt-2 block text-right text-xs text-muted">{text.length.toLocaleString("ko-KR")} / {field.config.maxLength.toLocaleString("ko-KR")}자</span> : null}</> : field.inputType === "SELECT" ? <FieldSelect id={id} required={required} value={text} onChange={(event) => onChange(event.target.value)}><option value="">선택하지 않음</option>{field.config.options?.map((option) => <option key={option}>{option}</option>)}</FieldSelect> : <><FieldInput id={id} required={required} type={field.id === "EMAIL" ? "email" : field.inputType === "TEL" ? "tel" : field.inputType === "URL" ? "url" : "text"} inputMode={field.id === "PHONE" ? "tel" : field.id === "EMAIL" ? "email" : undefined} autoComplete={field.id === "PHONE" ? "tel" : field.id === "EMAIL" ? "email" : undefined} maxLength={field.id === "PHONE" ? 13 : undefined} value={text} placeholder={field.config.placeholder} aria-invalid={Boolean(fieldError) || undefined} aria-describedby={fieldError ? `${id}-error` : undefined} onBlur={() => setTouched(true)} onChange={(event) => onChange(field.id === "PHONE" ? formatKoreanPhone(event.target.value) : event.target.value)} />{fieldError ? <span id={`${id}-error`} role="alert" className="mt-2 block text-sm font-medium text-fail">{fieldError}</span> : null}</>}</label>;
}
