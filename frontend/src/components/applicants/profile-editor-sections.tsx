...
function CareerSection({ value, onChange }: { readonly value: readonly CareerEntry[]; readonly onChange: (value: ApplicantAnswerValue) => void }) {
  ...
  return <div className="mt-6"><p className="text-sm leading-6 text-muted">최근 경력부터 정리하면 지원서 검토 화면에서도 같은 순서로 보여요.</p><div className="mt-4 space-y-4">{value.map((career, index) => <article key={`${career.title}-${index}`} className="rounded-card border border-border bg-surface p-4"><div className="mb-3 flex items-center"><strong className="text-sm">경력 {index + 1}</strong><TextButton onClick={() => onChange(value.filter((_, candidate) => candidate !== index))} className="ml-auto px-3 text-fail hover:bg-fail-bg hover:text-fail">삭제</TextButton></div><div className="grid gap-3 sm:grid-cols-[100px_1fr_160px]"><label><span className="mb-1 block text-xs text-muted">연도</span><FieldInput type="number" value={career.year} onChange={(event) => patch(index, { year: Number(event.target.value) })} /></label><label><span className="mb-1 block text-xs text-muted">작품명</span><FieldInput value={career.title} onChange={(event) => patch(index, { title: event.target.value })} /></label><label><span className="mb-1 block text-xs text-muted">배역</span><FieldInput value={career.part} onChange={(event) => patch(index, { part: event.target.value })} /></label></div></article>)}{value.length === 0 ? <div className="rounded-card border border-dashed border-border bg-surface px-5 py-8 text-center text-sm text-muted">저장한 경력이 없어요. 경력이 없어도 프로필을 저장할 수 있습니다.</div> : null}<AddButton onClick={() => onChange([...value, { year: new Date().getFullYear(), title: "", part: "" }])} className="min-h-12 w-full">+ 경력 추가</AddButton></div></div>;
}
...
function CustomSection({ profile, values, removed, onChange, onRequestRemove }: Pick<SectionProps, "profile" | "values" | "removed" | "onChange" | "onRequestRemove">) {
  ...
  return <div className="mt-6 space-y-4">{answers.map((answer) => <article key={answer.key} className="rounded-card border border-border p-4"><div className="flex items-start gap-4"><div className="min-w-0 flex-1"><h3 className="font-semibold">{answer.label}</h3><p className="mt-1 text-xs text-muted">출처 · {answer.lastUsedPostingTitle ?? "이전 지원서"}</p></div><TextButton onClick={() => onRequestRemove(answer.key)} className="px-3 text-fail hover:bg-fail-bg hover:text-fail">삭제</TextButton></div><FieldTextarea value={typeof values[answer.key] === "string" ? values[answer.key] as string : answerValueText(values[answer.key]!)} onChange={(event) => onChange(answer.key, event.target.value)} rows={3} className="mt-4" /></article>)}</div>;
}
