...
export function ApplicantReviewDecision({
  ...
  const reviewLocked = board.role.allRoundsClosed
    || board.rounds.find((state) => state.round === board.round)?.closed === true;
  ...
  return (
    <aside className="rounded-card border border-border bg-card p-4 lg:sticky lg:top-16">
      <div className="flex flex-col gap-3">
        <div>
          <h2 className="text-base font-bold text-foreground">심사 결정</h2>
          ...
          {reviewLocked ? (
            ...
            return (
              <button
                ...
                className={`min-h-10 rounded-control border px-2.5 py-2 text-xs font-semibold transition-colors disabled:pointer-events-none disabled:opacity-60 ${
      ...
      <label className="mt-3 block border-t border-border-soft pt-3">
