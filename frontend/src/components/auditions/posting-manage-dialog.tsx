...
function EditPostingForm({ detail, onClose, onChanged }: { readonly detail: PostingManagementDetail; readonly onClose: () => void; readonly onChanged: () => void }) {
  ...
  return <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
    <DialogHeader id={TITLE_ID} title="공고 수정" />
    <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-6 md:px-6">
      {started ? <p className="rounded-card border border-warn-line bg-warn-soft p-4 text-sm leading-6 text-muted-strong">모집이 시작돼 일부 항목이 잠겼습니다. 모집 마감은 뒤로만 늘릴 수 있습니다.</p> : null}
      <CreateSection title="공고명" description="배우에게 가장 먼저 보이는 제목입니다."><CreateField label="공고명"><FieldInput required maxLength={255} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="예: 2026 하반기 주·조연 배우 모집" /></CreateField></CreateSection>
      ...
      <CreateSection title="모집 일정" description="공고를 게시한 시각부터 모집이 시작됩니다.">{detail.recruitmentStart ? <p className="mb-4 rounded-control border border-border-soft bg-surface px-4 py-3 text-sm leading-6 text-muted-strong"><strong className="text-foreground">모집 시작</strong><br />{detail.recruitmentStart.replace("T", " ")}</p> : <p className="mb-4 rounded-control border border-border-soft bg-surface px-4 py-3 text-sm leading-6 text-muted-strong">아직 게시 전입니다.</p>}<CalendarDateRangeField includeTime single start={recruitmentEnd} end="" onStartChange={setRecruitmentEnd} onEndChange={() => undefined} startLabel="모집 마감" /></CreateSection>
      <CreateSection title="연습 장소" description="공연장과 다른 경우에만 입력합니다."><PerformanceVenueField optional venueLabel="연습 장소명" addressLabel="연습 장소 주소" mapLabel="연습 장소 지도" venue={rehearsalVenue} address={rehearsalVenueAddress} onVenueChange={setRehearsalVenue} onAddressChange={setRehearsalVenueAddress} /></CreateSection>
      <CreateSection title="전형 일정" description="완료된 차수는 수정할 수 없습니다. 오디션 장소는 공개 공고에 표시됩니다."><AuditionScheduleEditor rounds={rounds} onChange={setRounds} lockedRounds={detail.lockedRounds} /></CreateSection>
