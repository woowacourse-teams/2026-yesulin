...
export function ActionBar() {
  const { board, filters, selected, saving, roundClosed, clearSelection, setStatus, openContacts } = useBoard();
  ...
  const actions = <>
    {roundClosed ? <span className="self-center whitespace-nowrap px-2 text-xs opacity-60">마감된 차수</span> : selectableStatuses(board.round).map((status) => <button key={status} type="button" disabled={saving} onClick={() => { void setStatus(ids, status); }} className={ACTION_CLASS}>{STATUS_LABELS[status]}</button>)}
