...
function EditPerformanceForm({ detail, postingCount, onClose, onChanged }: {
  ...
  return <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col"><DialogHeader id={TITLE_ID} title="공연 수정" subtitle="공고를 등록하기 전까지만 수정할 수 있습니다." /><div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-6 md:px-6"><CreateSection title="공연 기본 정보"><div className="grid grid-cols-[120px_minmax(0,1fr)] gap-4 md:grid-cols-[150px_1fr]"><PosterUploadField label="공연 포스터" value={posterUrl} onChange={setPosterUrl} onFileChange={setPosterFile} /><CreateField label="공연 제목"><FieldInput required maxLength={200} value={title} onChange={(event) => setTitle(event.target.value)} /></CreateField></div><div className="mt-5"><PerformanceVenueField optional venue={venue} address={venueAddress} onVenueChange={setVenue} onAddressChange={setVenueAddress} /></div></CreateSection><CreateSection title="공연 기간" description="종료일을 비우면 오픈런으로 저장됩니다."><CalendarDateRangeField start={performanceStart} end={performanceEnd} endOptional endOpenEnded onStartChange={setPerformanceStart} onEndChange={setPerformanceEnd} startLabel="공연 시작일" endLabel="공연 종료일" /></CreateSection><CreateSection title="배역"><PerformanceRoleEditor roles={roles} onChange={setRoles} /></CreateSection><CreateError id="performance-manage-error" message={formError} /></div><DialogFooter><SecondaryButton onClick={onClose}>취소</SecondaryButton><PrimaryButton type="submit" disabled={saving}>{saving ? "저장 중…" : "변경 사항 저장"}</PrimaryButton></DialogFooter></form>;
}
...
function EditLockedPerformancePeriod({ detail, postingCount, onClose, onChanged }: {
  ...
  return <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col"><DialogHeader id={TITLE_ID} title="공연 기간 확인" subtitle="연결된 공고가 있어 공연 기본 정보와 배역은 잠겨 있습니다." /><div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-6 md:px-6"><p className="rounded-card border border-warn/30 bg-warn-bg p-4 text-sm leading-6 text-muted-strong">공고 {postingCount}건이 연결돼 있습니다. 이미 제출된 지원서의 공연 기간은 바뀌지 않습니다.</p><CreateSection title="공연 기간" description="종료일을 비우면 오픈런으로 저장됩니다."><CalendarDateRangeField start={performanceStart} end={performanceEnd} endOptional endOpenEnded onStartChange={setPerformanceStart} onEndChange={setPerformanceEnd} startLabel="공연 시작일" endLabel="공연 종료일" /></CreateSection><CreateSection title="현재 배역"><PerformanceRoleReadOnlyList roles={detail.roleTemplates} /></CreateSection><CreateError id="performance-period-error" message={formError} /></div><DialogFooter><SecondaryButton onClick={onClose}>취소</SecondaryButton><PrimaryButton type="submit" disabled={saving}>{saving ? "저장 중…" : "공연 기간 저장"}</PrimaryButton></DialogFooter></form>;
}
