...
export function ApplicationFieldEditor({ fields, onChange }: {
  ...
  return <div className="space-y-5">
    ...
    {video ? <FieldGroup title="영상 링크" description="필요한 영상을 설명별로 추가해 주세요. 배우는 각 설명에 맞는 링크를 제출하며, 최대 10개까지 받을 수 있습니다.">
      <VideoRequirements value={video.config.videoRequirements ?? []} onChange={(requirements) => patch(video.id, { enabled: requirements.length > 0, required: requirements.length > 0, config: { ...video.config, videoRequirements: requirements, maxCount: 10 } })} />
    </FieldGroup> : null}
    <FieldGroup title="커스텀" description="질문 문구는 최대 255자, 배우가 작성하는 답변은 최대 2,000자입니다. 질문마다 필수 여부를 정할 수 있습니다.">
      {custom.map((field) => <div key={field.id} className="flex flex-wrap gap-2 sm:flex-nowrap"><FieldInput required maxLength={255} value={field.label} onChange={(event) => patch(field.id, { label: event.target.value.slice(0, 255) })} placeholder="예: 지원 동기를 적어 주세요." className="min-w-52 flex-1" /><RequirementSelect required={field.required} label={field.label || "커스텀 질문"} onChange={(required) => patch(field.id, { required })} /><button type="button" onClick={() => onChange(fields.filter((item) => item.id !== field.id))} className="min-h-11 rounded-control border border-border bg-card px-3 text-sm text-muted-strong hover:text-fail">삭제</button></div>)}
      <button type="button" onClick={addCustom} className="min-h-11 w-full rounded-control border border-dashed border-muted-soft bg-card px-3 py-2.5 text-sm font-semibold text-muted-strong hover:border-brand-line hover:bg-brand-soft hover:text-brand">텍스트 질문 추가</button>
    </FieldGroup>
  </div>;
}
