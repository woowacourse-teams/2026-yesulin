...
비로그인 상태에서 `지원서 작성` 또는 `지원서 이어쓰기`를 누르면 로그인 후 프로필을 불러올지, 로그인 없이 로컬 Draft로 작성할지 선택한다. 공연·공고 추가 폼도 마지막 변경 600ms 뒤 IndexedDB에 자동 저장하며 의도적으로 모달을 닫을 때 대기 중 변경을 즉시 저장한다. 같은 작성 폼을 다시 열면 복원하고 실제 생성 성공 뒤 삭제한다. 이 제작사 Draft는 현재 브라우저 전용이며 서버 공고 `DRAFT`나 다른 계정·기기에 동기화되지 않는다.
